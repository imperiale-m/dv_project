...
d3.csv('../data/healthExpenditure2.csv', d3.autoType)
  .then((data) => {
    function drawChart(filteredData) {
      ...
      const [minX, maxX] = d3.extent(filteredData, (d) => d.obs_value);
      ...
      const [minY, maxY] = d3.extent(filteredData, (d) => d.life);
    ...
    // group data by 'geo'
    const filteredData = data.filter((d) => d.country_name === 'Italy');
    ...
    setInterval(() => {
      if (targetNode.textContent !== previousValue) {
        ...
        const filteredData = data.filter((d) => d.country_name === newValue.toString());
        drawChart(filteredData);
      }
